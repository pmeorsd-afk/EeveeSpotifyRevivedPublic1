...
class AviSplashHandler: NSObject {
    ...
    func showSplash(in window: UIWindow) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            ...
            btnTel.addTarget(self, action: #selector(self.openTelegram), for: .touchUpInside)
            ...
            btnClose.addTarget(self, action: #selector(self.dismiss), for: .touchUpInside)
...
struct AviSplashGroup: HookGroup { }

class SplashWindowHook: ClassHook<UIWindow> {
    typealias Group = AviSplashGroup

    func makeKeyAndVisible() {
        orig.makeKeyAndVisible()

        if !AviSplashHandler.shared.hasShown {
            AviSplashHandler.shared.hasShown = true
            AviSplashHandler.shared.showSplash(in: target)
        }
    }
}
